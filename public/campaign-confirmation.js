...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        if (sendButton) {
            // Remove all previous click event listeners by replacing the button, but keep the same DOM node
            // Instead, use a simple trick: set onclick to null, then addEventListener
            sendButton.onclick = null;
            sendButton.addEventListener('click', enhancedStartSession);
        }
    }, 3000);
});
...
async function enhancedStartSession() {
    ...
    try {
        ...
        if (!window.appState.phoneNumbers || window.appState.phoneNumbers.length === 0 || !messageInput.value.trim()) {
