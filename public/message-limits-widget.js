...
class MessageLimitsWidget {
  ...
  async loadMessageStats() {
    try {
      const response = await fetch(`http://104.154.62.181:3001/api/message-limits?machineId=${encodeURIComponent(this.deviceId)}`);
  ...
  async checkMessageLimits(messageCount = 1) {
    try {
      const response = await fetch(`http://104.154.62.181:3001/api/message-limits/check?machineId=${encodeURIComponent(this.deviceId)}`, {
        method: 'POST',
  ...
  async trackMessageUsage(messageCount = 1, campaignId = null, contactCount = null) {
    try {
      ...
      const response = await fetch(`http://104.154.62.181:3001/api/message-limits/track?machineId=${encodeURIComponent(this.deviceId)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
