...
function findSystemChrome() {
    ...
    sendToUI('log', { level: 'warning', message: '⚠� No system browser found, using bundled Chromium' });
    return null;
}
...
function setupWhatsAppEventHandlers() {
    ...
    waClient.on('change_state', (state) => {
        ...
        if (state === 'CONFLICT' || state === 'UNLAUNCHED') {
            sendToUI('log', { level: 'warning', message: `⚠� WhatsApp state: ${state}` });
...
function cleanupWhatsAppClient() {
    ...
    if (waClient) {
        try {
            ...
            sendToUI('log', { level: 'warning', message: `⚠� Cleanup warning: ${error.message}` });
        }
        waClient = null;
    }
    isReady = false;
...
function initializeAntiBanFeatures() {
    sendToUI('log', { level: 'info', message: '🛡� Initializing anti-ban features...' });
    cleanMessageTracker();
...
function getDeviceId() {
    try {
        ...
        const rawId = `${cpuInfo}|${hostname}|${platform}|${arch}|${release}`;
    ...
    function handleWhatsAppStatus(result) {
        ...
        switch (result.status) {
            ...
            case 'error':
                ...
                break;
            default:
                sendToUI('log', { level: 'info', message: `ℹ� WhatsApp status: ${result.message}` });
                break;
        }
    }
...
ipcMain.handle('pause-session', async () => {
    ...
    sendToUI('log', { level: 'info', message: '�� Campaign paused by user.' });
...
ipcMain.handle('continue-session', async () => {
    ...
    sendToUI('log', { level: 'info', message: '▶� Campaign resumed by user.' });
...
ipcMain.handle('stop-session', async () => {
    ...
    sendToUI('log', { level: 'warning', message: '�� Campaign stop requested by user.' });
...
ipcMain.handle('start-session', async (event, data) => {
    ...
    if (!waClient) {
        sendToUI('log', { level: 'warning', message: '⚠� WhatsApp client not initialized. Attempting to initialize...' });
    ...
    sendToUI('log', { level: 'info', message: `🛡� Batch size: ${ANTI_BAN_CONFIG.batchSize}, Delays: ${ANTI_BAN_CONFIG.minDelay}-${ANTI_BAN_CONFIG.maxDelay}ms` });
    ...
    function waitIfPaused() {
        if (sessionControl.stopped) {
            sendToUI('log', { level: 'warning', message: '�� Campaign stopped by user.' });
            return Promise.resolve(false);
        }
        if (!sessionControl.paused) return Promise.resolve(true);
        sendToUI('log', { level: 'info', message: '�� Campaign paused. Waiting for resume...' });
        return new Promise(resolve => {
            pausedPromiseResolve = () => {
                sendToUI('log', { level: 'info', message: '▶� Campaign resumed.' });
                resolve(true);
            };
        });
    }
    ...
    try {
        for (let i = 0; i < numbers.length; i++) {
            ...
            if (i > 0 && i % ANTI_BAN_CONFIG.batchSize === 0) {
                ...
                if (sessionControl.stopped) {
                    sendToUI('log', { level: 'warning', message: `�� Campaign stopped during batch break.` });
                    break;
                }
            }
            ...
            if (delay > 0) {
                ...
                if (sessionControl.stopped) {
                    sendToUI('log', { level: 'warning', message: `�� Campaign stopped during delay.` });
                    break;
                }
            }
...
// Removed stray 'ive' statement causing ReferenceError
// Device ID handler
